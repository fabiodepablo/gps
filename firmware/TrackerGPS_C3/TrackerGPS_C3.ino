...
void initDisplay() {
  ...
  u8g2.setFont(u8g2_font_ncenB10_tr); u8g2.drawStr(20, 30, "Tracker C3");
...
void initGPRS(String apn, String user, String pass) {
  if (dispData.gsmStatus == "Modulo Desconectado") return;
  
  dispData.gsmStatus = "Conectando red...";
  sendAT("AT+SAPBR=3,1,\"APN\",\"" + apn + "\"");
  sendAT("AT+SAPBR=1,1", 3000); 
  String ip = sendAT("AT+SAPBR=2,1");
  if(ip.indexOf("+SAPBR: 1,1,\"") != -1 && ip.indexOf("0.0.0.0") == -1) { dispData.gsmStatus = "GPRS Ok"; dispData.dbConnected = true; }
  else { dispData.gsmStatus = "Error GPRS o SIM"; dispData.dbConnected = false; }
}
...
// -------------------------------------------------------------------
const int BTN_PRG = 9; // BOOT button de ESP32-C3 Super Mini
const int BAT_PIN = 2; // ADC Generico en C3
